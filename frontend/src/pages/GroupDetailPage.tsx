...
function Initials({ name, size = 32 }: { name: string; size?: number }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: size, height: size, borderRadius: '50%', background: '#dbeafe', color: '#1d4ed8', fontSize: size * 0.38, fontWeight: 700, flexShrink: 0 }}>{ini}</span>
  );
}
...
function SectionActions({ onCancel, onSave, saving }: { onCancel: () => void; onSave: () => void; saving: boolean }) {
  return (
    <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
      ...
      <button className="btn-primary" onClick={onSave} disabled={saving} style={{ fontSize: 13 }}>{saving ? 'Saving…' : 'Save Changes'}</button>
    </div>
  );
}

function InfoPanel({ group, users, canEdit, onSaved, updateInfo }: {
  ...
  users: UserSelectDto[]; canEdit: boolean; onSaved: (msg: string) => void;
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', border: `1px solid ${editing ? '#bfdbfe' : '#e5e7eb'}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>User Group Information</span>
        {canEdit && !editing && (
          <button onClick={startEdit} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4 }}><PencilIcon /></button>
        )}
      </div>
      ...
      {!editing ? (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 24px' }}>
            <div style={{ marginBottom: 18 }}><InfoLabel label="Group ID" /><div style={{ fontSize: 14, color: '#111827' }}>{group.groupCode}</div></div>
            <div style={{ marginBottom: 18 }}><InfoLabel label="Status" /><span className={`badge badge-${group.status.toLowerCase()}`}>• {group.status}</span></div>
            <div style={{ marginBottom: 18, gridColumn: '1 / -1' }}><InfoLabel label="Group Name" /><div style={{ fontSize: 14, color: '#111827' }}>{group.groupName}</div></div>
          </div>
          <div style={{ marginBottom: 18 }}><InfoLabel label="Group Email ID" /><div style={{ fontSize: 14, color: '#111827' }}>{group.groupEmailId || '–'}</div></div>
          ...
          <div><InfoLabel label="Group Description" /><div style={{ fontSize: 14, color: '#111827' }}>{group.groupDesc || '–'}</div></div>
        </>
      ) : (
        <>
          ...
          <SectionActions onCancel={() => setEditing(false)} onSave={save} saving={updateInfo.isPending} />
        </>
      )}
    </div>
  );
...
function MembersPanel({ memberIds, memberDetails, users, canEdit, onSaved, syncMembers }: {
  memberIds: number[]; memberDetails: { userId: number; fullName: string; email: string }[];
  users: UserSelectDto[]; canEdit: boolean; onSaved: (msg: string) => void;
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', border: `1px solid ${editing ? '#bfdbfe' : '#e5e7eb'}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        {canEdit && !editing && (
          <button onClick={() => { setSelected(memberIds); setSearch(''); setEditing(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4 }}><PencilIcon /></button>
        )}
      </div>

      <div style={{ position: 'relative', marginBottom: 14 }}>
        <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af', fontSize: 14 }}>⌕</span>
        <input placeholder={editing ? 'Search users to add…' : 'Enter member name you are looking for'} value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 28, fontSize: 13, width: '100%' }} />
      </div>

      {!editing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {memberDetails.length === 0 && <div style={{ color: '#9ca3af', fontSize: 13, padding: '8px 0' }}>No members in this group.</div>}
          {memberDetails
            .filter(m => !search || m.fullName.toLowerCase().includes(search.toLowerCase()) || m.email.toLowerCase().includes(search.toLowerCase()))
            .map(m => (
              <div key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, background: '#f9fafb', border: '1px solid #f3f4f6' }}>
        ...
        <>
          <div style={{ maxHeight: 300, overflowY: 'auto', border: '1px solid #e5e7eb', borderRadius: 6 }}>
            {filteredUsers.map(u => {
              ...
              return (
                <div key={u.id} onClick={() => toggle(u.id)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', cursor: 'pointer', background: checked ? '#eff6ff' : '#fff', borderBottom: '1px solid #f3f4f6' }}>
          ...
          <SectionActions onCancel={() => setEditing(false)} onSave={save} saving={syncMembers.isPending} />
        </>
      )}
    </div>
  );
...
function GroupRightsPanel({ allScreens, savedPermissions, editing, canEdit, onStartEdit, onSave, onCancel, saving }: {
  allScreens: ScreenPermissionDto[]; savedPermissions: ScreenPermissionDto[];
  editing: boolean; canEdit: boolean; onStartEdit: () => void;
  onSave: (p: ScreenPermissionSaveDto[]) => void; onCancel: () => void; saving: boolean;
}) {
    ...
    <div style={{ background: '#fff', borderRadius: 10, padding: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.07)', border: `1px solid ${editing ? '#bfdbfe' : '#e5e7eb'}` }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>You can grant access to the system module and their sub-module for the user group</div>
        </div>
        {canEdit && !editing && (
          <button onClick={() => { const m: Record<number, ScreenPermissionSaveDto> = {}; for (const p of mergedPermissions) m[p.screenId] = { ...p }; setDraft(m); onStartEdit(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280', padding: 4 }}><PencilIcon /></button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginTop: 16, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: 340 }}>
          <span style={{ position: 'absolute', left: 9, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af', fontSize: 14 }}>⌕</span>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Enter module or feature name" style={{ paddingLeft: 28, fontSize: 13, width: '100%' }} />
        </div>
        {editing && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn-secondary" onClick={onCancel} disabled={saving} style={{ fontSize: 13 }}>Cancel</button>
            <button className="btn-primary" onClick={() => onSave(Object.values(draft))} disabled={saving} style={{ fontSize: 13 }}>{saving ? 'Saving…' : 'Save Changes'}</button>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          return (
            <div key={modName} style={{ border: `1px solid ${editing ? '#bfdbfe' : '#e5e7eb'}`, borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', background: open ? '#eff6ff' : '#fff', userSelect: 'none' }}>
                <div onClick={() => setOpenMods(prev => { const n = new Set(prev); n.has(modName) ? n.delete(modName) : n.add(modName); return n; })} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '11px 14px', flex: 1, cursor: 'pointer' }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 20, height: 20, borderRadius: '50%', border: '2px solid #93c5fd', color: '#1d4ed8', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{open ? '−' : '+'}</span>
                  <span style={{ fontWeight: 700, fontSize: 14, color: '#1d4ed8' }}>{modName}</span>
                </div>
                {editing && (
                  <div style={{ display: 'flex', gap: 6, padding: '0 14px' }}>
                    <button type="button" onClick={() => applyToModule(screens.map(s => s.screenId), makeReadOnly)} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 4, border: '1px solid #d1d5db', background: '#fff', color: '#374151', cursor: 'pointer', fontWeight: 500 }}>Read Only</button>
                    <button type="button" onClick={() => applyToModule(screens.map(s => s.screenId), makeAllAccess)} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 4, border: '1px solid #0B5AA0', background: '#0B5AA0', color: '#fff', cursor: 'pointer', fontWeight: 500 }}>All Access</button>
                  </div>
                )}
              </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: '#f9fafb' }}>
                        ...
                        {PERM_COLS.map(c => <th key={c.key} style={{ padding: '7px 6px', textAlign: 'center', fontWeight: 600, fontSize: 11, color: '#6b7280', minWidth: 65, whiteSpace: 'nowrap' }}>{c.label}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {screens.map(s => (
                          ...
                          {PERM_COLS.map(col => {
                            ...
                            return (
                              <td key={col.key} style={{ textAlign: 'center', padding: '7px 4px' }}>
                                {editing ? (
                                  <input type="checkbox" checked={val} onChange={e => update(s.screenId, col.key, e.target.checked)} style={{ width: 15, height: 15, cursor: 'pointer', accentColor: '#0B5AA0' }} />
                                ) : (
        ...
        {modules.size === 0 && (
          <div style={{ padding: 24, textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>
            {allScreens.length === 0 ? 'Loading screens…' : !editing && !search ? 'No permissions assigned to this group.' : 'No screens match your search.'}
          </div>
        )}
      </div>
    </div>
...
export default function GroupDetailPage() {
  ...
  return (
    <div>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, background: toast.ok ? '#dcfce7' : '#fee2e2', color: toast.ok ? '#16a34a' : '#dc2626', padding: '12px 20px', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.12)', fontSize: 14, fontWeight: 500 }}>
          {toast.msg}
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        <InfoPanel group={group} users={users} canEdit={perms.edit} onSaved={showToast} updateInfo={updateInfo} />
        <MembersPanel memberIds={group.members.map(m => m.userId)} memberDetails={group.members} users={users} canEdit={perms.edit} onSaved={showToast} syncMembers={syncMembers} />
      </div>
