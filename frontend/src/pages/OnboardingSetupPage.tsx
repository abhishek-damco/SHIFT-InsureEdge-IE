        ...
        <div className="card">
          {!tokenValid ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: '#374151', marginBottom: 16 }}>This onboarding link is invalid or has expired (links are valid for 24 hours). Please contact your administrator to resend the invitation.</p>
            </div>
          ) : done ? (
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
              <p style={{ color: '#374151', marginBottom: 16 }}>Your account is ready. Welcome to InsureEdge!</p>
              <button className="btn-primary" onClick={() => navigate('/login')}>Sign In</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              <p style={{ color: '#4b5563', marginBottom: 20, fontSize: 14 }}>Please set a password for your account. Your setup link is valid for 24 hours.</p>
              ...
              <button type="submit" className="btn-primary" style={{ width: '100%' }} disabled={loading}>{loading ? 'Activating...' : 'Activate Account'}</button>
            </form>
          )}
        </div>
      </div>
