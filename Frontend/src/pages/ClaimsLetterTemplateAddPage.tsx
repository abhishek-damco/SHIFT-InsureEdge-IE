...
import { letterTemplatesApi, CreateDocumentPayload } from '../api/letterTemplates';
...
export default function ClaimsLetterTemplateAddPage() {
  ...
  const { data: codeData } = useQuery({
    queryKey: ['letter-template-next-code'],
    queryFn: letterTemplatesApi.getNextCode,
    staleTime: 0,
  });
  const templateCode = codeData?.code ?? '...';
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return letterTemplatesApi.create({
        templateName: templateName.trim(),
        active,
        templateCategory: category,
        subjectLine: subjectLine.trim(),
        description: description.trim(),
        lineOfBusiness: 'E&S Homeowners',
        insuranceType: 'Speciality Lines',
        states,
        documents: doc
          ? [{ documentName: doc.fileName, documentFile: doc.fileName, documentContent: docContent, version: doc.version, effectiveStartDate: doc.effectiveStartDate || null, effectiveEndDate: doc.effectiveEndDate || null } as CreateDocumentPayload]
          : [],
      });
    },
    onSuccess: () => {
    ...
    onError: (err) => {
      console.error('[LetterTemplate] Save failed:', err);
    },
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'Arial, sans-serif', fontSize: '14px', color: '#333' }}>
      ...
      <div style={{ padding: '20px 24px 0' }}>
        <h2 style={{ margin: '0 0 4px', fontSize: '20px', fontWeight: '600', color: '#1a1a2e' }}>Add Template</h2>
      ...
      <div style={{ flex: 1, display: 'flex', gap: '16px', padding: '0 24px', overflow: 'auto' }}>
        ...
        <div style={{ flex: '0 0 430px', border: '1px solid #d1d5db', borderRadius: '6px', padding: '20px', background: '#fff', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#e0f2f1', border: '1px solid #b2dfdb', borderRadius: '4px', padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ color: '#00695c', fontWeight: '500', fontSize: '13px' }}>Template ID - {templateCode}</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '13px', fontWeight: '500' }}>{doc?.version ?? '0.0'}</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '16px' }}>
            <div><div style={SUBLBL}>Line of Business</div><div style={ROVAL}>E&amp;S Homeowners</div></div>
            <div><div style={SUBLBL}>Insurance Type</div><div style={ROVAL}>Speciality Lines</div></div>
          </div>
        ...
        <div style={{ flex: 1, border: '1px solid #d1d5db', borderRadius: '6px', padding: '20px', background: '#fff', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              ...
              disabled={!!doc}
              style={{ backgroundColor: doc ? '#b0bec5' : '#1565c0', color: '#fff', border: 'none', borderRadius: '4px', padding: '7px 14px', cursor: doc ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '500' }}>
              + Add Document
            </button>
          </div>
                ...
                <tr>
                  <td style={{ ...DTC, textAlign: 'center' }}>
                    <span style={{ display: 'inline-flex', gap: '10px', alignItems: 'center' }}>
                      <span title="View" onClick={handleViewDoc} style={{ cursor: 'pointer', color: '#555', fontSize: '16px' }}>&#128065;</span>
                      <span title="Remove" onClick={() => setDoc(null)} style={{ cursor: 'pointer', color: '#555', fontSize: '18px', lineHeight: 1 }}>&#x2297;</span>
                    </span>
                  </td>
                  <td style={DTC}>{doc.fileName}</td>
                  <td style={DTC}>{doc.version}</td>
                  <td style={DTC}>{fmtDate(doc.effectiveStartDate)}</td>
                  <td style={DTC}>{fmtDate(doc.effectiveEndDate)}</td>
                </tr>
              </tbody>
            </table>
          )}
      ...
      {showDocModal && (
        <AddDocumentModal
          onAdd={(entry) => { setDoc(entry); setShowDocModal(false); }}
      ...
      {/* View Document Modal */}
      {viewModal && doc && (
        <DocViewerModal file={doc.file} onClose={() => setViewModal(false)} />
      )}
    </div>
  );
}
...
function DocViewerModal({ file, onClose }: { file: File; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (ext === 'pdf') {
      // PDFs render natively in iframe — handled separately
    ...
    (async () => {
      ...
      // Verify ZIP magic bytes before passing to docx-preview
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '8px', width: '880px', maxWidth: '95vw', height: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', background: '#e8e8e8', position: 'relative' }}>
          ...
          {/* DOCX — docx-preview rendered content */}
          {!isPdf && (
